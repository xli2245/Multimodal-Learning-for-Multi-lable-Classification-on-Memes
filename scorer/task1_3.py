...
def _read_gold_and_pred(pred, gold):
    """
    ...
    if not isinstance(gold, list):
        with open(gold, encoding='utf-8') as gold_f:
    ...
    if not isinstance(pred, list):
        with open(pred, encoding='utf-8') as pred_f:
...
def evaluate(pred, gold, CLASSES):
    ...
    """
    pred_labels, gold_labels = _read_gold_and_pred(pred, gold)
